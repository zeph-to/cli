...
// Tool-access preamble — pi only. Pi has no MCP support, so the zeph_*
// tools the shared core names never appear in its tool list; the CLI is
// the same wire (ask.ts / notify) so the mapping is semantic, not a fork.
...
//
// Not named *_HOOKS on purpose: templates.test.ts auto-collects every
// *_HOOKS export and JSON.parses string configs — these are TS source
// files, not JSON. Their quiet-default guard lives in installer.test.ts.
// Both embed NOTIFY_CMD / remoteHookCmd via JSON.stringify so the shell
// strings can never drift from the other agents'.
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("agent_settled", async (_event, ctx) => {
    await sh(${JSON.stringify(NOTIFY_CMD)}, ctx.cwd);
  });
...
export const ZephPlugin = async ({ directory }) => ({
  ...
  event: async ({ event }) => {
    ...
    await new Promise((resolve) => {
      const child = spawn("sh", ["-c", NOTIFY], { cwd: directory, stdio: "ignore" });
      child.on("error", resolve);
