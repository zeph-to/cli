...
The receiver only starts once the Device Keypair has loaded — both its routes (LAN Auth)
authenticate with it.

### LAN Auth (`lan-auth.ts`, `lan-auth.vectors.json`)
The one security boundary of local transfer, and the contract four implementations share
(`src/lan-auth.ts` here and, byte-exact, in `@zeph-to/mcp-server`, the Kotlin and Swift
uploaders). From the ECDH secret: `macKey = HKDF-SHA256(secret, info "zeph-lan-v1")` and
`metaKey = HKDF-SHA256(secret, info "zeph-lan-v1-meta")`;
`mac = HMAC-SHA256(macKey, method|path|senderDeviceId|transferId|timestamp|nonce|sha256(body))`
in `X-Zeph-Lan-{Sender,Transfer,Timestamp,Nonce,Mac}`; the upload adds `X-Zeph-Lan-Meta` =
`base64(iv || AES-256-GCM(metaKey, {fileName, fileType, fileSize, iv, deviceKeyMap, transferId}))`
over a raw ciphertext body. The receiver's 200 / 201 carries `X-Zeph-Lan-Receipt` =
`HMAC-SHA256(macKey, "zeph-lan-receipt|" + mac)`; a sender counts a transfer delivered only on a
receipt that verifies, because any host answering on the address could say 201. The transport is plain HTTP, so the meta is sealed: a sniffer sees a
sender device id and a transfer id, never a file name or the account's device list. Clock window
±5 min; nonces are recorded only after a MAC verified, per sender, 1000 each, refused not evicted
when full, in memory (ADR-0013 accepts the restart window). The implementations stay in step by
the vectors file — generated by an independent Python implementation — not by reading each other.
Change the canonical string or the info strings → regenerate the vectors → every implementation
goes red until it follows.

### LAN Sender (`lan-sender.ts`)
`pickLanTarget` + `tryLanDelivery`, byte-exact in `@zeph-to/mcp-server` (whose `zeph_file` is
today's only caller). Eligible: one target device with a `publicKey`, a `lan` endpoint and
`isOnline`, and a sealed file. Ping (1.5 s) → upload (30 s + 1 s per MiB) → delivered only on a
verified receipt; every other outcome is `{ delivered: false, reason }` and the caller relays.
`lan-integration.test.ts` runs it against a real receiver.

### Landing Zone (`lan-receiver.ts`, `device-directory.ts`)
`~/.zeph/attachments/lan/<transferId>/<fileName>`: where the receiver leaves a decrypted local
transfer until its push record (`lanDeliveredTo` = this device) arrives over the WebSocket and
`claimLanFile` moves it to the Downloads Folder. An outsider — anything not authenticated as a
device of this account — gets one answer for every attempt: 401, empty (shape, clock,
replayed nonce, unknown sender, and on an upload a meta that does not open — so a
genuine device id without its key gets no further either). Only an authenticated account device
sees the statuses that say anything about this machine, all still before the body: 409
(transferId in progress or landed), 503 (two uploads in flight), 411 / 413 (no length / over
1 GiB), 400 (meta shape, or not wrapped for this device). The MAC and the GCM tag are checked when the last byte is in; either failing removes
the transfer dir and spends no nonce. No wall-clock body budget — a 60 s idle timeout instead. A
pre-body refusal still reads and discards the body for 1 s so the client sees its status, not a
reset. There is no pre-auth request budget: before the MAC an outsider buys only a header parse and
two map lookups (derived keys cached per public key), so there is nothing to drain. Sockets are capped at 64 with a 5 s header timeout, so a flood
exhausts local transfer, not the daemon. The
sender's registered key comes from `device-directory.ts`, a cached `GET /devices` (60 s TTL, one
refetch per 10 s on a miss or a failure); a device removed from the account is refused within that
TTL. Orphans (upload landed, push never came) are swept by the same 24 h `gcAttachments` pass, one
level down, matched by the zone's full path.
