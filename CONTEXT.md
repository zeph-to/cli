...
(this file, its byte-exact twin in `mcp-server/src/lan-auth.ts`, the Kotlin and Swift
...
over a raw ciphertext body. The transport is plain HTTP, so the meta is sealed: a sniffer sees a
